 ...
 *
 * `loading` renders the standard spinner ahead of the children — the caller keeps control of the
 * label, so swapping text while busy stays a one-line ternary at the call site.
 ...
 * @param {boolean} [props.loading] Shows the spinner ahead of the children.
 * @param {boolean} [props.fullWidth] Stretches the button to the container width.
 * @param {ReactNode} [props.leftIcon] Rendered before the children.
 * @param {ReactNode} [props.rightIcon] Rendered after the children.
 * @param {string} [props.className] Extra classes; conflicting utilities override the variant's.
 * @returns {JSX.Element} The button.
 */
export default function Button({ variant = 'bare', size = 'none', loading = false, fullWidth = false, leftIcon, rightIcon, className = '', type = 'button', children, ...rest }: { loading?: boolean; fullWidth?: boolean; leftIcon?: ReactNode; rightIcon?: ReactNode } & VariantProps<typeof buttonVariants> & ButtonHTMLAttributes<HTMLButtonElement>)
{
    return (
        <button
            type={ type }
