...
- `setValueEncrypted`/`getValueEncrypted` — AES-GCM 256 with an **Argon2id** key derived from the passphrase, at the same `m=65536 / t=3 / p=1 / len=32` cost as the unlock hash in [password.ts](src/core/password.ts) (this used to be PBKDF2-SHA256 at 102400 iterations; it is not any more). Fresh random salt+IV per write, stored as base64 `{ salt, iv, cipher, kdf }`. The passphrase is never persisted; a wrong passphrase surfaces as a thrown error (GCM auth tag). The payload does carry a `kdf: 'argon2id'` marker — but **it is written and never read**: `getValueEncrypted` derives with Argon2id unconditionally, so the marker records intent rather than buying compatibility, and changing the cost parameters still breaks every stored blob exactly as it does for the unlock hash.
...
**What it does unlock** is `tauri-plugin-http`, which was tried twice and reverted twice purely because it tied a feature to Tauri. That objection is void. It would make favicons byte-cacheable, which is now a cost/benefit call rather than a rule. The second reason it kept coming up — reading the Nura explorer without waiting on that server's CORS headers — is spent: the headers arrived (see *Known, unfixed*), which is the better fix and the one that cost nothing here. Nothing has been changed on the strength of it yet.
...
- **The Argon2id parameters are a broken compatibility contract.** Verification recomputes the hash with whatever [password.ts](src/core/password.ts) says today and compares it against one written under yesterday's constants, so a wallet created before they were raised past `m=32768 / t=2 / p=1` can no longer be unlocked. Fixing it properly means storing the parameters alongside the hash and verifying with the set the hash was written under, then re-hashing at the new cost after a successful unlock. Note that the encrypted-mnemonic payload in `storage.ts` has the same weakness — it now writes a `kdf: 'argon2id'` marker, but nothing reads it back, so it buys nothing yet — and a real fix covers both. Until then, treat these constants as frozen.
- **Nura's activity list is empty because the chain is empty. The CORS block that used to cause it is fixed.** `explorer.nurachain.net/api` carried no `Access-Control-Allow-Origin` and answered the preflight with `405`, so the webview fetched the body and then refused to hand it to script. That was fixed on the server, and verified on 2026-08-16: the `GET` now carries `ACAO: *` and `OPTIONS` answers `204`. What is left is not a defect. `txlist` replies `{"status":"0","message":"No transactions found","result":[]}`, and twenty-one blocks sampled across the whole range hold **no transactions at all** — so `readAction` returns an empty array with no `notice`, and the tab shows its empty state rather than `Dashboard.Activity.Unavailable`, which is the honest answer. **Nothing in [hook/history.ts](src/hook/history.ts) is wrong and nothing there needs changing**; test the pipeline against Ethereum, whose Blockscout has always answered. Re-check the header with `curl -sD- -o/dev/null -H "Origin: http://tauri.localhost" "https://explorer.nurachain.net/api?module=account&action=txlist&address=0x0"` — it is worth re-checking, because if it ever regresses there is still no client-side substitute: the node is go-ethereum with no `ots_*` and no `trace_filter`, `eth_getLogs` is capped at 10 000 blocks and covers token transfers only, and `/api/v2`, `/graphiql` and `/api/v1/graphql` are all 404.
- **Nura Chain is a moving target — confirm its ids against the node before believing a bug report.** It answered `eth_chainId` with `0x3fc` (**1020**) at a head of ~67 000 on a `dev` build of go-ethereum, where it had been 1010 past block 311 284: the chain was re-launched under a new id. [network.ts](src/core/network.ts) was corrected to 1020 on 2026-08-16. Reads never noticed the mismatch — `getProvider` passes the id with `staticNetwork`, so `eth_chainId` is never called — but every signature is EIP-155 bound to it, so a stale constant fails **sending** and nothing else. Per-chain state filed under the old id (`Wallet.Tokens`, `Wallet.TokensHidden`, both `Record<chainId, …>`) was deliberately left unmigrated: the contracts it names do not exist on the new chain.
