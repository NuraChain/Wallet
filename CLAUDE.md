...
[src-tauri/src/lib.rs](src-tauri/src/lib.rs) exposes **no `#[tauri::command]` at all** — it registers plugins (single-instance and fs on desktop only, os and store everywhere) and the tray handler. Wallet derivation, hashing, encryption and UI are all frontend. **There is no `http` plugin**: every request goes through the webview's own `fetch`, so a server that sends no `Access-Control-Allow-Origin` is a wall the frontend cannot climb (site favicons are the standing example). Adding native capability means a command in `lib.rs` *plus* the matching permission in **each** per-platform capability block.
...
- **Nura's activity list is empty because the chain is empty, not because of CORS.** The explorer's missing `Access-Control-Allow-Origin` was fixed server-side and verified on 2026-08-16. `txlist` now answers `{"status":"0","message":"No transactions found","result":[]}` and sampled blocks hold no transactions at all, so the empty state is the honest answer. **Nothing in [hook/history.ts](src/hook/history.ts) needs changing** — test the pipeline against Ethereum, whose Blockscout has always answered. If the header ever regresses there is no client-side substitute: the node is go-ethereum with no `ots_*` and no `trace_filter`, `eth_getLogs` is capped at 10 000 blocks, and the v2/GraphQL endpoints are all 404.
