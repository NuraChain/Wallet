 ...
 *
 * Renders a plain `div` carrying one of the pairings above, so a caption cannot drift onto a different
 * size or colour by being retyped. Anything a call site needs on top — `truncate`, `font-mono`,
 * alignment, margins — rides in through `className`, which `cn` lets win over the variant.
 *
 ...
 *
 * Headings that carry document structure (`h1`, `p` in the intro slides) are deliberately left as the
 * semantic elements they are rather than routed through here.
 * @param {object} props Component props.
 * @param {string} [props.variant] Which pairing to render: `caption`, `captionStrong`, `body`, `bodyMuted`, `title`, `heading`.
 ...
 */
export default function Text({ variant = 'caption', text, className = '', children, ...rest }: { text?: string; className?: string; children?: ReactNode } & VariantProps<typeof textVariants> & Omit<HTMLAttributes<HTMLDivElement>, 'className' | 'children'>)
{
    return (
        <div className={ cn(textVariants({ variant }), className) } { ...rest }>
